    ...
    $MainScriptBlock =
    {
        ...
        if ((Get-Service -Name DNS -ErrorAction SilentlyContinue) -and (Get-Service -Name DNS).Status -eq 'Running')
        {
            ...
            @(
                ...
                @{ Name = 'adfs';                    Type = 'A';      Data = "$DomainNetworkId.60" }
                @{ Name = 'certauth.adfs';           Type = 'A';      Data = "$DomainNetworkId.60" }
                @{ Name = 'enterpriseregistration';  Type = 'A';      Data = "$DomainNetworkId.60" }
        ...
        if ((Get-Service -Name DHCPServer).Status -eq 'Running')
        {
            ...
            $DhcpReservations =
            @(
                @{ Host = 'DC02';        Name = "DC02.$DomainName";        IPAddress = "$DomainNetworkId.20"; }
                @{ Host = 'AS01';        Name = "AS01.$DomainName";        IPAddress = "$DomainNetworkId.50"; }
                @{ Host = 'ADFS01';      Name = "ADFS01.$DomainName";      IPAddress = "$DomainNetworkId.60"; }
        ...
        [ordered]@{
            ...
            @{
                Version = '26H1'
                Workstation = 'Windows 11 26H1 (28000)'
                WorkstationEndOfSupport = '2029-03-13'
            }
        }
        ...
        foreach ($t in @('DC', '0', '1', '2'))
        {
            ...
            foreach ($Computer in (Get-ADObject -Filter "Name -like '*' -and ObjectCategory -eq 'Computer'" -SearchBase "OU=Tier $t,OU=$DomainName,$BaseDN" -SearchScope Subtree ))
            {
                ...
                @{
                    ...
                    Members         = $( if ($t -eq '2') { @("Tier 2 - Users") })
                }
            }
        }
            ...
            switch($t)
            {
                ...
                default
                {
                    $AuthNPolicyUsers = @(Get-ADGroupMember -Identity "Tier $t - Admins") +
                                        @(Get-ADGroupMember -Identity "Tier $t - Users").Where({$_.distinguishedName -notin $RemoteAccessUsers.distinguishedName}) # Exclude Remote Access Users

                                        $AuthNPolicyComputers = @(Get-ADGroupMember -Identity "Tier $t - Computers").Where({$_.distinguishedName -notin $RemoteAccessComputers.distinguishedName}) # Exclude Remote Access Computers
                }
            }
        ...
        foreach ($Pol in $AuthNPolicies)
        {
            ...
            foreach ($UserDN in $Pol.Users)
            {
                ...
                if (-not $GetAssignedPolicy -or $GetAssignedPolicy -notmatch $ADAuthNPolicy.DistinguishedName -and
                    (ShouldProcess -Message "Assigning `"$UserCN`" to `"$($Pol.Name) Policy`"" @VerboseSplat))
                {
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUQl6c8DDfvud3fFfgArn+kPRv
# //qgghhFMIIFBzCCAu+gAwIBAgIQdFzLNL2pfZhJwaOXpCuimDANBgkqhkiG9w0B
...
# BgorBgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFg